...
export function createStore(): Store {
	const store: Store = data({
		...
		getSortedUsers() {
			if (store.usersOrder === "name")
				return store.users.slice().sort(cmp<User, string>((x) => x.name.first));
			if (store.usersOrder === "id")
				return store.users.slice().sort(cmp<User, string>((x) => x.id));

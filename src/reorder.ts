...
export const App = () => {
  ...
  const setCount = (e: Event) => {
    state.count = Math.round(parseInt((e.target! as HTMLInputElement).value, 10));
  };
  ...
  const rotateBackward = () => {
    let { items, count } = state,
      len = items.length;
    state.items = items.slice(len-count, len).concat(items.slice(0, len-count));
  };
  ...
  return () => html`
    <div class="reorder-demo">
      <header>
        ...
        <label><input type="checkbox" @click=${toggleKeys} checked=${state.useKeys} /> use keys?</label>
        <label><input type="number" step="1" min="1" style=${{ width: '3em' }} @input=${setCount} value=${state.count} /> count</label>
      </header>
      <ul>
